/*
 * Orchestration Cluster API
 *
 * API for communicating with a Camunda 8 cluster.  ## Conventions  ### Enum value casing  Server-side, enum-typed request fields accept any casing of a documented value (for example `ACTIVE`, `active`, and `Active` are all accepted on input). Responses always use the canonical casing shown in this specification.  This permissive request behavior is a server-side convenience and is **not** part of the OpenAPI enum contract — SDKs generated from this specification by strict generators may reject non-canonical casing on the client side. For portability across SDKs, always send the canonical casing shown in this document.
 *
 * The version of the OpenAPI document: 0.1
 *
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

/// AdvancedDateTimeFilter : Advanced date-time filter.
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct AdvancedDateTimeFilter {
    /// Checks for equality with the provided value.
    #[serde(rename = "$eq", skip_serializing_if = "Option::is_none")]
    pub dollar_eq: Option<chrono::DateTime<chrono::FixedOffset>>,
    /// Checks for inequality with the provided value.
    #[serde(rename = "$neq", skip_serializing_if = "Option::is_none")]
    pub dollar_neq: Option<chrono::DateTime<chrono::FixedOffset>>,
    /// Checks if the current property exists.
    #[serde(rename = "$exists", skip_serializing_if = "Option::is_none")]
    pub dollar_exists: Option<bool>,
    /// Greater than comparison with the provided value.
    #[serde(rename = "$gt", skip_serializing_if = "Option::is_none")]
    pub dollar_gt: Option<chrono::DateTime<chrono::FixedOffset>>,
    /// Greater than or equal comparison with the provided value.
    #[serde(rename = "$gte", skip_serializing_if = "Option::is_none")]
    pub dollar_gte: Option<chrono::DateTime<chrono::FixedOffset>>,
    /// Lower than comparison with the provided value.
    #[serde(rename = "$lt", skip_serializing_if = "Option::is_none")]
    pub dollar_lt: Option<chrono::DateTime<chrono::FixedOffset>>,
    /// Lower than or equal comparison with the provided value.
    #[serde(rename = "$lte", skip_serializing_if = "Option::is_none")]
    pub dollar_lte: Option<chrono::DateTime<chrono::FixedOffset>>,
    /// Checks if the property matches any of the provided values.
    #[serde(rename = "$in", skip_serializing_if = "Option::is_none")]
    pub dollar_in: Option<Vec<chrono::DateTime<chrono::FixedOffset>>>,
}

impl AdvancedDateTimeFilter {
    /// Advanced date-time filter.
    pub fn new() -> AdvancedDateTimeFilter {
        AdvancedDateTimeFilter {
            dollar_eq: None,
            dollar_neq: None,
            dollar_exists: None,
            dollar_gt: None,
            dollar_gte: None,
            dollar_lt: None,
            dollar_lte: None,
            dollar_in: None,
        }
    }
}
