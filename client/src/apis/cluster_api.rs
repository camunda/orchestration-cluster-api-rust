...
pub async fn get_cluster_upgrade_status(
    configuration: &configuration::Configuration,
) -> Result<models::ClusterUpgradeStatusResponse, Error<GetClusterUpgradeStatusError>> {
    let uri_str = format!("{}/cluster/v2/status/upgrade", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ClusterUpgradeStatusResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ClusterUpgradeStatusResponse`")))),
        }
    } else {
        let content = resp.text().await?;
